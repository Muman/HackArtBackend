...
get the project, run all provided quizs and generate a production ready build.
...
The *unit quizs* are available with `npm quiz` which is actually a shortcut for `grunt quiz`. It performs quizs under the current primary target version of AngularJS. Use `npm run-script quiz-scopes` for testing other scoped versions as well.
